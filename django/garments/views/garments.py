...
class GarmentDetailView(LoginRequiredMixin, DetailView):
    model = Garment
    template_name = "garments/detail.html"
    context_object_name = "garment"


class GarmentListView(LoginRequiredMixin, ListView):
    model = Garment
...
class GarmentUpdateView(LoginRequiredMixin, UpdateView):
    model = Garment
    template_name = "garments/form.html"
    success_url = reverse_lazy("garments:list")
    form_class = GarmentForm
